...
function Board({ width, height }) {
    const { isRunning, setIsRunning, grid, setGrid,
        liveCellsCount, setLiveCellsCount,
        frequency, setFrequency, gridDeath, setGridDeath } = useContext(GameContext);
    ...
    const changeFreq = useCallback((freq) => {
        if (freq < 50 || freq > 2000) {
            setFreqError("Please input a valid frequency from 50 to 2000ms inclusive");
        } else {
            setFreqError("")
            setFrequency(freq)
        }
